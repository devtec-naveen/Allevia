...
def symptom_prompting(current_symptoms_input, specialization):
    '''
    The primary symptom prompting machine learning routine for Allevia. Flow is
    symptoms -> convert to technical -> suggest technical affiliated symptoms 
    -> filter to remove from current technical symptoms -> layman -> return
    Can be issue if conversion to technical doesn't match the technical term in 
    associated symptom prompting files.
    Inputs:
    ...
    specialization_to_filename = {"GEN": "/opt/lampp/htdocs/allevia/webroot/python_doc/primarycare_json_v1.0.txt", "OBGYN": "/opt/lampp/htdocs/allevia/webroot/python_doc/primarycare_json_v1.0.txt", 
                                   "ORTHO":"/opt/lampp/htdocs/allevia/webroot/python_doc/orthopedics_json_v2.0.txt", "ORTHO_SPINE":"/opt/lampp/htdocs/allevia/webroot/python_doc/orthopedics_json_v2.0.txt","COVID": "/opt/lampp/htdocs/allevia/webroot/python_doc/covid_json_v1.0.txt"}
    ...
    # Restrict prevalences to those in acute symptoms.
    acute_prevalence = {k:v for k, v in acute_prevalence.items() if k in acute_symptoms}
    ...
    # Patients are only qualified to answer symptoms with these types.
    allowed_ask_types = {"SUBJ", "HIST", "RISK", "LOCA", "CHAR", ""} # ,"PHYS", "DIAG", "TMNT", "ID"}
    potential_asked_symptoms = [ordered_symptoms[current_top_disease][i] if 'no ' not in ordered_symptoms[current_top_disease][i] else 
                                ordered_symptoms[current_top_disease][i][3:] for i in range(len(ordered_symptoms[current_top_disease])) if 
                                "type" in acute_symptoms[current_top_disease][ordered_symptoms[current_top_disease][i]] and acute_symptoms[current_top_disease][ordered_symptoms[current_top_disease][i]]["type"] in allowed_ask_types]

    for symptom in potential_asked_symptoms:
        # Symptom not in current_symptom_input may cause problems down the line. Not a permanent solution.
        if symptom not in all_asked_symptoms and symptom not in current_symptoms and symptom not in current_symptoms_input:
