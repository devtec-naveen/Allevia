...
class DemoController extends AppController
{
     ...
     public function beforeFilter(Event $event)
    {
     ...
     $this->Auth->allow(['getConditions','sepecializationDetail','healthQuestion','updateFirstLastName','updateScheduleData','scheduleSettings','addOrgUrl','getClinics','getVisitReasons','getPreAppointment','getAllVisits','providerConfigSettings','providerTelehealthTemplate','cronicDatabase','updateClProviderId','updateClGroupId','emailSettings','showquestionnaire']);
    ...
    public function emailSettings(){
        ...
        $emailTemplates = $EmailTemplateTlb->find('all')->where(['slug IN' => ['pre_appointment_form_link','pre_appointment_reminder','telehealth_reminder','telehealth_reminder_before_x_time','','api_notification']])->toArray();
        //pr($emailTemplates);die;
    ...
    {
      $this->viewBuilder()->setLayout('front');
      $this->ChiefCompliantQuestionnaire = $this->loadModel('ChiefCompliantQuestionnaire');
      $getallQuestionData = $this->ChiefCompliantQuestionnaire->find('all')->contain(['questionnairetype'])->where(['ChiefCompliantQuestionnaire.step_id' =>'19'])->toArray();
      $this->set('getallQuestionData',$getallQuestionData);
