                        ...
                        <div class="header">
                            <h2>
                             ...
                             <ol class="breadcrumb breadcrumb-col-blue header-dropdown m-r--5">
                                ...
                                <li class="active">Transaction History List</li>
                            </ol>
                        </div>
                                ...
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    <thead>
                                     ...
                                     <tbody>
                                        ...
                                        <?php $user = array();
                                        if(!empty($value->schedule->user_id)){
                                         $user = $this->general->authuser($value->schedule->user_id);   
                                     }?>    
                                        <tr>    
                                          <td><?= $this->Number->format($i) ?></td>
                                          <!-- <td><?php if(!empty($user->first_name))
                                        echo  h($this->CryptoSecurity->decrypt( base64_decode($user->first_name), SEC_KEY)); ?></td>
                                        <td><?php if(!empty($user->last_name))
                                        echo  h($this->CryptoSecurity->decrypt( base64_decode($user->last_name), SEC_KEY)); ?></td> -->
                                          <td><?= h($value->amount) ?></td>
                                          <td><?= h($value->paymentIntent_id) ?></td>
                                          <td><?= h(ucfirst($value->status)) ?></td>
